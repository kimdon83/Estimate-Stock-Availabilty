...
df_sumBOseq = df_result.groupby('mtrl').agg({'BOseq': 'sum'})
df_sumBOseq.reset_index(inplace=True)
df_sumBOseq["StartDate"] = ''
df_sumBOseq["ox"] = ""
df_sumBOseq["Extra_Available_Qty"]=0

for index, row in df_sumBOseq.iterrows():
    if row.BOseq > 0:
        id = (df_result["mtrl"] == row.mtrl) & (df_result["BOseq"] == 1)
        df_sumBOseq.loc[index, "StartDate"] = df_result.loc[id,
                                                            "StartDate"].values[0]
        df_sumBOseq.loc[index, "ox"] = 'x'
    elif row.BOseq < 0:
        df_sumBOseq.loc[index, "StartDate"] = today
        df_sumBOseq.loc[index, "ox"] = 'x'
    elif row.BOseq == 0:
        lastday = df_total.loc[df_total.mtrl == row.mtrl].iloc[-1]
        # fcst=lastday.fcstD
        if sum(df_total.loc[df_total.mtrl == row.mtrl, "fcstD"]) == 0:
            # inventory>0 but no fcst
            df_sumBOseq.loc[index, "StartDate"] = '2100-01-01'
            df_sumBOseq.loc[index, "ox"] = 'o'
        else:
            if lastday.fcstD == 0:
                fcst = np.average(df_total.loc[(df_total.mtrl == row.mtrl) & (
                    df_total.fcstD > 0), "fcstD"].values)
            ...
            df_sumBOseq.loc[index, "StartDate"] = datetime.strftime(
                bo, '%Y-%m-%d')
            df_sumBOseq.loc[index, "ox"] = 'o'
    else:
        print(row)
...
df_sumBOseq[["mtrl", "StartDate", 'ox']].to_csv(BOdateloc, index=False)
