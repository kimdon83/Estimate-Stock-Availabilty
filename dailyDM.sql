...
AS (
    ...
    WHERE act_date BETWEEN DATEADD(DD, - DAY(GETDATE()), GETDATE()) AND DATEADD(MM, 7, DATEADD(DD, - DAY(GETDATE
                            ()), GETDATE()))
...
AS (
    ...
    FROM (
        ...
        FROM [ivy.mm.dim.date]
        WHERE thedate BETWEEN DATEADD(DD, - DAY(GETDATE()), GETDATE()) AND DATEADD(MM, 7, DATEADD(DD, - DAY(
                                GETDATE()), GETDATE()))
        ) T1
